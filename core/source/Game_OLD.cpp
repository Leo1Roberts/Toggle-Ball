...
bool Game_OLD::inEditor = false;
bool Game_OLD::toggled = false;
bool Game_OLD::levelComplete = false;
bool Game_OLD::viewLocked = true;
bool slowMotion = false;

Smoother Game_OLD::smoother;

Level Game_OLD::level;
std::vector<Obstacle> Game_OLD::obstacles;
float Game_OLD::arenaWidth, Game_OLD::arenaHeight;
Plane Game_OLD::arenaBounds[4];
Plane Game_OLD::arenaBackground;
Ball_OLD Game_OLD::ball;
Ball_OLD Game_OLD::ballOutline;

void Game_OLD::setBallPos(const vec3& pos) {
	ball.pos = pos;
...
void Game_OLD::setupObjShaders() {
	Shaders::load();

	vec3 groundColor = colorToLinear({0.3f, 0.3f, 0.3f});
	vec3 skyColor = colorToLinear({85.0f / 255.0f, 110.0f / 255.0f, 128.0f / 255.0f});
	vec3 sunColor = colorToLinear({1.0f, 1.0f, 0.9f});
...
void Game_OLD::rotateView(const vec2& dxy) { rotateView(dxy.x, dxy.y); }

void Game_OLD::rotateView(float dx, float dy) {
...
void Game_OLD::moveViewOrigin(float dx, float dy) {
...
GLFWwindow* Game_OLD::window;

bool forceExit;
void Game_OLD::deleteGame() {
	...
	if (!forceExit && !changesAreSaved()) {
		...
		confirmExit();
#else
android_app* Game_OLD::androidApp;
EGLDisplay Game_OLD::eglDisplay;
EGLSurface Game_OLD::eglSurface;
EGLContext Game_OLD::eglContext;
...
void Game_OLD::updateHalfHeight() {
...
void Game_OLD::updateRenderArea() {
#if defined(PLATFORM_DESKTOP)
	int width, height;
...
void Game_OLD::restartLevel(int _) {
	if (inEditor) return;
...
int Game_OLD::dialogue;
void Game_OLD::requestDialogue(int d) {
	switch (d) {
	case DLG_NONE:
		dialogue = DLG_NONE;
		break;
...
#if defined(PLATFORM_DESKTOP)
void Game_OLD::confirmExit(int _) {
	forceExit = true;
	glfwSetWindowShouldClose(window, GLFW_TRUE);
}
#endif

bool Game_OLD::justOpenedSaveAsDialogue = false;
std::string Game_OLD::saveAsName;
void Game_OLD::saveAs(int _) {
...
void Game_OLD::resetEditorView(int _) {
...
void Game_OLD::ballObstacleCollision(Ball_OLD* pBall, Obstacle* g, const vec3& normal, float separation) {
...
short Game_OLD::checkBallObstacleCollision(Ball_OLD* pBall, bool getIndex, bool onlyRim, bool onlySection) {
	short obIndex = -1;
...
void Game_OLD::updateEditorObstacles() {
...
void Game_OLD::drawInventoryPanel() {
...
void Game_OLD::drawPropertiesPanel() {
...
void Game_OLD::drawBall(const Ball_OLD& b) {
	drawObject(Meshes::ball.get(), b.texture, b.pos, b.rot, b.scale);
}

void Game_OLD::drawPlane(const Plane& p) {
	drawObject(Meshes::plane.get(), p.texture, p.pos, p.rot, p.scale);
}

void Game_OLD::drawObject(const Mesh<ObjectVertex>* model, const Texture* texture, const vec3& pos, const mat3& rot, const vec3& scale) {
...
void Game_OLD::drawObstacleOutline(const Obstacle& g) {
	buildScaledWorldMatrix(&worldMat, g.getRot(), g.getPos(), vec3(1));
	mat4 projectionMatrix = worldMat * viewMat * projMat;
	Shaders::outline->setMat4("uProjectionFull", projectionMatrix);
...
void Game_OLD::drawObstacleDomains() {
	glEnable(GL_DEPTH_TEST);
	glDepthFunc(GL_LESS);
...
vec3 Game_OLD::getPointerWorldPos(float x, float y) {
...
vec3 Game_OLD::getPointerWorldPos(const vec2& pos) { return getPointerWorldPos(pos.x, pos.y); }

std::vector<std::string> Game_OLD::getLevelList() {
	return AssetManager::getFileList("levels", ".lvl");
}

void Game_OLD::updateArena() {
...
void Game_OLD::loadLevel(const Level& newLevel, bool resetView) {
	level = newLevel;
	levelComplete = false;
...
std::string Game_OLD::levelString;
bool resetUndoBufferFlag = false;

void Game_OLD::selectLevel(const std::string& name) {
	Level newLevel;
	bool loaded;
...
std::string Game_OLD::pendingLevel;

void Game_OLD::selectPendingLevel(int _) {
	selectLevel(pendingLevel);
	requestDialogue(DLG_NONE);
}

void Game_OLD::selectLevelCallback(int levelIndex) {
	pendingLevel = getLevelList()[levelIndex];
	requestDialogue(DLG_CONFIRM_SWITCH);
}

short Game_OLD::savedIndex;
void Game_OLD::saveLevel(int _) {
#if defined(PLATFORM_DESKTOP)
...
bool Game_OLD::allArrowKeysUp(int exceptKey) {
...
void Game_OLD::moveObjectsWithArrowKey(int key, int keyAction) {
...
void Game_OLD::handleKeyInput(GLFWwindow* window, int key, int scancode, int keyAction, int mods) {
...
void Game_OLD::handleCharInput(GLFWwindow* window, unsigned int c) {
	if (TextBoxManager::focusedBoxIndex >= 0 && c < CHAR_MAX)
		TextBoxManager::typed((char)c);
}

void Game_OLD::handleCursorPosInput(GLFWwindow* window, double xpos, double ypos) {
...
void Game_OLD::handleCursorEnterEvent(GLFWwindow* window, int entered) {
	if (entered)
		Cursor::visible = true;
	else {
...
void Game_OLD::handleMouseButtonInput(GLFWwindow* window, int mouseButton, int mouseButtonAction, int mods) {
	switch (mouseButton) {
...
void Game_OLD::handleScrollInput(GLFWwindow* window, double xoffset, double yoffset) {
...
short Game_OLD::focus;
void Game_OLD::findNewFocus() {
...
bool Game_OLD::selection[] = {false};
void Game_OLD::selectObject(short index) {
	focus = index;
	if (!ctrlDown && !selection[index]) // Deselect all other objects
		memset(selection, false, sizeof(selection));
	selection[index] = true;
}
void Game_OLD::deselectObject(short index) {
	selection[index] = false;
	if (focus == index)
		findNewFocus();
}

bool Game_OLD::limiting[] = {false};

bool Game_OLD::ctrlDown, Game_OLD::shiftDown, Game_OLD::altDown;
bool Game_OLD::leftMouseDown, Game_OLD::middleMouseDown, Game_OLD::rightMouseDown;

byte Game_OLD::action;
byte Game_OLD::actionMod;
bool Game_OLD::actionIsStateless;
vec2 Game_OLD::actionStartPointerPos;
vec3 Game_OLD::actionStartPointerWorldPos;
vec3 Game_OLD::actionVector;
short Game_OLD::pressedObIndex;
float Game_OLD::initialPointerObDist;
...
vec2 Game_OLD::selectBoxTL, Game_OLD::selectBoxBR;
...
bool Game_OLD::blobWasPressed;
void Game_OLD::blobPressed(int blobActionMod) {
	if (action == ACTION_NONE) {
		action = ACTION_SHAPE;
...
void Game_OLD::changePropertyInput() { // Called whenever text box focus changes
	if (TextBoxManager::anythingChanged)
		finishAction();
...
void Game_OLD::updatePropertyInput(byte property) {
...
void Game_OLD::moveBallByFull(const vec3& vector) {
	moveBallTo(undoBuffer[bufferIndex].level.ballPos + vector);
}
void Game_OLD::moveBallBy(const vec3& vector) {
	moveBallTo(ball.pos + vector);
}
void Game_OLD::moveBallTo(const vec3& pos) {
	setBallPos(pos);
	level.ballPos = pos;
}

void Game_OLD::moveObstacleByFull(short index, const vec3& vector) {
...
void Game_OLD::moveObstacleTo(short index, const vec3& pos) { // TODO: remove this function
...
void Game_OLD::rotateObstacleBy(short index, float angle) {
...
void Game_OLD::rotateObstacleTo(short index, float angle) { // TODO: remove this function
...
std::vector<ObstacleDefinition> Game_OLD::clipboard;

void Game_OLD::addObstacle(bool isStraight, bool isGoal, byte stateType) {
...
void Game_OLD::addObstacleCallback(int data) {
	addObstacle(data & 0x100, data & 0x200, data & 0xff);
}

void Game_OLD::deleteObstacles() {
...
void Game_OLD::duplicateObstacles() {
	short numSelected = 0;
	for (short i = 0; i < (short)obstacles.size(); i++)
...
void Game_OLD::cutObstacles() {
	copyObstacles();
	deleteObstacles();
}

void Game_OLD::pasteObstacles() {
...
void Game_OLD::updateBallOutline() {
	float factor = 1 + OUTLINE_WIDTH * HALF_HEIGHT * 2;
	ballOutline.radius = ball.radius * factor;
	ballOutline.scale.set(factor);
}

void Game_OLD::updateAllOutlines() {
	updateBallOutline();
	for (Obstacle& g: obstacles)
		g.createOutlineModel();
}

vec2 Game_OLD::pointerPos;
Ball_OLD Game_OLD::pointerBall = Ball_OLD();
Ball_OLD Game_OLD::pointerBallWide = Ball_OLD();

short Game_OLD::bufferIndex;
short Game_OLD::lastBufferIndex;
short Game_OLD::selectionBufferIndex;
UndoNode_old Game_OLD::undoBuffer[MAX_UNDO];
