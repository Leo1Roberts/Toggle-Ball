...
DrawOperation::DrawOperation(EditorScene& scene, const Camera& camera, const EditorQuickSettings& quickSettings, TriggerType trigger, glm::vec2 initialPlanarPosition, float minorRadius, std::optional<float> tangentAngle)
	...
	manipulateCapOperation = std::make_unique<ManipulateRightCapOperation>(scene, camera, quickSettings, trigger, initialPlanarPosition, obstacle, tangentAngle);
}